...
def load_data():
    ...
    # 파일 크기 체크 (LFS 포인터 여부 확인용)
    file_size = os.path.getsize(data_path)
    if file_size < 1024 * 100:  # 100KB 미만이면 포인터 파일일 가능성 높음
        st.error(f"데이터 파일({data_path})의 크기가 너무 작습니다 ({file_size} bytes). GitHub LFS 설정 문제로 실제 데이터가 아닌 포인터 파일만 업로드된 것 같습니다.")
        st.info("해결 방법: deployment_guide.md의 '트러블슈팅' 섹션을 확인하여 실제 데이터를 다시 업로드해 주세요.")
        st.stop()
